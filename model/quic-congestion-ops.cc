...
QuicCongestionOps::QuicCongestionOps(const QuicCongestionOps& sock)
    : TcpNewReno(sock)
{
    NS_LOG_FUNCTION(this);
}
...
void
QuicCongestionOps::UpdateRtt(Ptr<TcpSocketState> tcb, Time latestRtt, Time ackDelay)
{
    NS_LOG_FUNCTION(this);
    Ptr<QuicSocketState> tcbd = dynamic_cast<QuicSocketState*>(&(*tcb));
    NS_ASSERT_MSG(tcbd != nullptr, "tcb is not a QuicSocketState");
    ...
    {
        Time rttVarSample = Time(std::abs((tcbd->m_smoothedRtt - latestRtt).GetDouble()));
...
void
QuicCongestionOps::OnPacketAcked(Ptr<TcpSocketState> tcb, Ptr<QuicSocketTxItem> ackedPacket)
{
    NS_LOG_FUNCTION(this);
    Ptr<QuicSocketState> tcbd = dynamic_cast<QuicSocketState*>(&(*tcb));
    NS_ASSERT_MSG(tcbd != nullptr, "tcb is not a QuicSocketState");
...
bool
QuicCongestionOps::InRecovery(Ptr<TcpSocketState> tcb, SequenceNumber32 packetNumber)
{
...
void
QuicCongestionOps::OnPacketAckedCC(Ptr<TcpSocketState> tcb, Ptr<QuicSocketTxItem> ackedPacket)
{
    NS_LOG_FUNCTION(this);
    Ptr<QuicSocketState> tcbd = dynamic_cast<QuicSocketState*>(&(*tcb));
    NS_ASSERT_MSG(tcbd != nullptr, "tcb is not a QuicSocketState");
...
void
QuicCongestionOps::OnPacketsLost(Ptr<TcpSocketState> tcb,
                                 std::vector<Ptr<QuicSocketTxItem>> lostPackets)
{
    ...
    // Start a new recovery epoch if the lost packet is larger than the end of the previous recovery
    // epoch.
...
void
QuicCongestionOps::OnRetransmissionTimeoutVerified(Ptr<TcpSocketState> tcb)
{
    NS_LOG_FUNCTION(this);
    Ptr<QuicSocketState> tcbd = dynamic_cast<QuicSocketState*>(&(*tcb));
    NS_ASSERT_MSG(tcbd != nullptr, "tcb is not a QuicSocketState");
