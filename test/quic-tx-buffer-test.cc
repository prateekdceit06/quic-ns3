...
QuicTxBufferTestCase::DoRun()
{
     ...
     */
    Simulator::Schedule(Seconds(0.0), &QuicTxBufferTestCase::TestNewBlock, this);
     ...
     */
    Simulator::Schedule(Seconds(0.0), &QuicTxBufferTestCase::TestPartialAck, this);
     ...
     *
     */
    Simulator::Schedule(Seconds(0.0), &QuicTxBufferTestCase::TestAddBlocks, this);
    /*
     * Edge cases of the block adding function:
     * -> add 1 block
...
QuicTxBufferTestCase::TestRetransmission()
{
    ...
    QuicSubheader sub =
        QuicSubheader::CreateStreamSubHeader(1, 0, p1->GetSize(), false, true, false);
    p1->AddHeader(sub);
    txBuf.Add(p1);
    ...
    std::vector<Ptr<QuicSocketTxItem>> acked =
        txBuf.OnAckUpdate(tcbd, largestAcknowledged, additionalAckBlocks, gaps);
    ...
    NS_TEST_ASSERT_MSG_EQ(acked.at(0)->m_packetNumber,
                          SequenceNumber32(1),
    ...
    sub = QuicSubheader::CreateStreamSubHeader(1, 1200, p2->GetSize(), false, true, false);
    p2->AddHeader(sub);
    txBuf.Add(p2);
    ...
    sub = QuicSubheader::CreateStreamSubHeader(1, 2400, p3->GetSize(), false, true, false);
    p3->AddHeader(sub);
    txBuf.Add(p3);
    ...
    acked = txBuf.OnAckUpdate(tcbd, largestAcknowledged, additionalAckBlocks, gaps);
    ...
    NS_TEST_ASSERT_MSG_EQ(lostPackets.at(0)->m_packetNumber,
                          SequenceNumber32(2),
                          "TxBuf gets the wrong lost packet ID");
    NS_TEST_ASSERT_MSG_EQ(txBuf.BytesInFlight(),
                          2400,
...
QuicTxBufferTestCase::TestRejection()
{
    ...
    QuicSubheader sub =
        QuicSubheader::CreateStreamSubHeader(1, 0, outPkt->GetSize(), false, true, false);
    ...
    NS_TEST_ASSERT_MSG_EQ(pos, true, "Failed to add packet");
    NS_TEST_ASSERT_MSG_EQ(socketTxBuf.Available(),
                          4800 - outPkt->GetSize(),
    ...
    sub = QuicSubheader::CreateStreamSubHeader(1, 2400, outPktMore->GetSize(), true, true, false);
    outPktMore->AddHeader(sub);
    NS_TEST_ASSERT_MSG_EQ(outPktMore->GetSize(),
                          2400 + sub.GetSerializedSize(),
                          "Wrong packet size");
    ...
    NS_TEST_ASSERT_MSG_EQ(pos, true, "Failed to add packet");
    NS_TEST_ASSERT_MSG_EQ(streamTxBuf.Available(),
                          14400 - sub.GetSerializedSize(),
                          "Wrong available data size");
    NS_TEST_ASSERT_MSG_EQ(streamTxBuf.AppSize(),
                          3600 + sub.GetSerializedSize(),
                          "Wrong buffer size");
}
...
QuicTxBufferTestCase::TestNewBlock()
{
    ...
    sub = QuicSubheader::CreateStreamSubHeader(1, 0, p2->GetSize(), false, true, false);
    p2->AddHeader(sub);
    txBuf.Add(p2);

    ptx = txBuf.NextSequence(1200, SequenceNumber32(2));
    NS_TEST_ASSERT_MSG_EQ(ptx->GetSize(),
                          1200,
    ...
    // Expecting 3000 (added, including QuicSubheader 4) - 1200 (extracted, including QuicSubheader
    // 4)
    ...
    Ptr<Packet> p3 = Create<Packet>(1196);
    sub = QuicSubheader::CreateStreamSubHeader(1, 0, p3->GetSize(), false, true, false);
    p3->AddHeader(sub);
    txBuf.Add(p3);
    Ptr<Packet> p4 = Create<Packet>(1196);
    sub = QuicSubheader::CreateStreamSubHeader(1, 0, p4->GetSize(), false, true, false);
    p4->AddHeader(sub);
    txBuf.Add(p4);
...
QuicTxBufferTestCase::TestPartialAck()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(lost.empty(), true, "TxBuf detects a non-existent loss");
    ...
    pkts.push_back(1);
    uint32_t i = 0;
    for (auto acked_it = acked.begin(); acked_it != acked.end(); ++acked_it, i++)
    {
        NS_LOG_LOGIC("Hello");
        NS_TEST_ASSERT_MSG_EQ((*acked_it)->m_packetNumber.GetValue(),
                              pkts.at(i),
...
QuicTxBufferTestCase::TestAckLoss()
{
    ...
    std::vector<Ptr<QuicSocketTxItem>> lost = txBuf.DetectLostPackets();
    NS_TEST_ASSERT_MSG_EQ(acked.size(),
                          5,
                          "TxBuf does not correctly detect the number of ACKed packets");
    ...
    NS_TEST_ASSERT_MSG_EQ(lost.at(0)->m_packetNumber.GetValue(),
                          2,
...
QuicTxBufferTestCase::TestSetLoss()
{
    ...
    NS_TEST_ASSERT_MSG_EQ(lost.size(), 1, "TxBuf cannot set the correct number of lost packets");
    NS_TEST_ASSERT_MSG_EQ(lost.at(0)->m_packetNumber,
                          SequenceNumber32(4),
                          "TxBuf gets the wrong lost packet ID");
    ...
    NS_TEST_ASSERT_MSG_EQ(lost.size(), 3, "TxBuf cannot set the correct number of lost packets");
    NS_TEST_ASSERT_MSG_EQ(lost.at(0)->m_packetNumber,
                          SequenceNumber32(1),
                          "TxBuf gets the wrong lost packet ID");
    NS_TEST_ASSERT_MSG_EQ(lost.at(1)->m_packetNumber,
                          SequenceNumber32(2),
                          "TxBuf gets the wrong lost packet ID");
    NS_TEST_ASSERT_MSG_EQ(lost.at(2)->m_packetNumber,
                          SequenceNumber32(4),
                          "TxBuf gets the wrong lost packet ID");
...
class QuicTxBufferTestSuite : public TestSuite
{
  public:
        ...
        : TestSuite("quic-tx-buffer", Type::UNIT)
    {
        ...
        AddTestCase(new QuicTxBufferTestCase, Duration::QUICK);
    }
};
